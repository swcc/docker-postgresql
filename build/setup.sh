...
if [ ! -f /var/lib/postgresql/9.2/main/PG_VERSION ] && [ "$DATABASE_NAME" ] && [ "$DATABASE_USER" ] && [ "$DATABASE_PASSWORD" ]
then
    # Create postgres data directory
    rm -rf   /var/lib/postgresql/9.2/main
    mkdir -p /var/lib/postgresql/9.2/main
    chown -R postgres:postgres /var/lib/postgresql/
    /sbin/setuser postgres /usr/lib/postgresql/9.2/bin/initdb /var/lib/postgresql/9.2/main/

    # Start postgresql
    /usr/bin/pg_ctlcluster "9.2" main start
    ...
    # Stop postgresql
    /usr/bin/pg_ctlcluster "9.2" main stop
fi
